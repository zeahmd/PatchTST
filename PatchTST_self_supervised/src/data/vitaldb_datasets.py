...
def compute_spectrogram(eeg_signal, fs=128, nperseg=128, noverlap=0):
    ...
    Sxx = (Sxx - np.nanmean(Sxx)) / (np.nanstd(Sxx) + 1e-5)  # Normalize
    return Sxx
...
class EEGSegmentDataset(Dataset):
    def __init__(self, data_dir, dataset, segment_sec=20, eeg_rate=128, emg_rate=1, stride_sec=20, mode='pretrain', split='train'):
    ...
    def create_supervised_index(self):
        ...
        for case_path in tqdm(case_dirs, desc=f"Reading {self.split} dataset"):
            ...
            # Step 4: compute spectrogram if needed
            if self.dataset in ['eeg_freq', 'eeg_time_freq']:
                Sxx = compute_spectrogram(eeg, fs=self.eeg_rate, nperseg=128, noverlap=0)

            # Step 5: segment data and create index
            length = len(eeg)
            for start in range(0, length, self.stride_len):
                ...
                # Step 6: segment EMG, BIS, and Spectrogram
                ...
                if self.dataset in ['eeg_freq', 'eeg_time_freq']:
                    Sxx_seg = Sxx[:, start_sec:end_sec] if end_sec <= Sxx.shape[1] else Sxx[:, start_sec:]
