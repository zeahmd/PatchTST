...
class EEGSegmentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.mode == 'pretrain':
            if self.dataset == 'eeg_time':
                eeg_seg, emg_avg = self.index[idx]
                ...
                return eeg_tensor, eeg_tensor, emg_avg_tensor
            elif self.dataset == 'eeg_freq':
                Sxx_seg, emg_avg = self.index[idx]
                ...
                emg_avg_tensor = torch.tensor(emg_avg, dtype=torch.float32)     #
                if not self.use_emg:
                    return Sxx_tensor, Sxx_tensor
                return Sxx_tensor, Sxx_tensor, emg_avg_tensor
            elif self.dataset == 'eeg_time_freq':
                raise NotImplementedError("Pretraining for 'eeg_time_freq' is not implemented yet.")
            ...
            if self.dataset == 'eeg_time':
                ...
                return eeg_tensor, bis_level_tensor, emg_avg_tensor
            elif self.dataset == 'eeg_freq':
                Sxx_seg, emg_avg, bis_level = self.index[idx]
                Sxx_tensor = torch.tensor(Sxx_seg, dtype=torch.float32) # (freq_bins, time_bins)
                # IMPORTANT: Before flattening, change shape: (freq_bins, time_bins) -> (time_bins, freq_bins)
                Sxx_tensor = Sxx_tensor.permute(1, 0)  # (time_bins, freq_bins)
                ##############################################################################################
                ...
                bis_level_tensor = bis_level_tensor.long() # (batch, 1)
                if not self.use_emg:
                    return Sxx_tensor, bis_level_tensor
                return Sxx_tensor, bis_level_tensor, emg_avg_tensor
            elif self.dataset == 'eeg_time_freq':
...
if __name__ == "__main__":
    ...
    dataset = EEGSegmentDataset('/home/woody/iwso/iwso204h/vitaldb/cleaned_data', dataset='eeg_freq', mode='finetune', split='train')
