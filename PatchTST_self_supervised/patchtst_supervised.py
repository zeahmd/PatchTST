...
parser.add_argument('--use_weighted_loss', type=int, default=0, help='whether to use weighted loss for imbalanced data')
...
parser.add_argument('--use_emg', type=int, default=1, help='whether to use emg as additional input feature')
# Optimization args
...
# args.save_model_name = f"patchtst_supervised_{args.dataset}_normlosses_nlayers{args.n_layers}_segment_sec{args.segment_sec}_patch_len{args.patch_len}_num_patch{args.num_patch}_epochs{args.epochs}"
# args.save_path = f"saved_models/patchtst_supervised/{args.dataset}/emg_supervision_exps"
# args.save_path = "saved_models/baseline"
# args.save_model_name = f"{args.dataset}_baseline_useemg={args.use_emg}"
if args.is_hyperopt:
    args.save_path += '_hyperopt'
...
def train_func(lr=args.lr):
    ...
    if args.use_weighted_loss:
        print('---------------- Using weighted loss with weights! ----------------')
...
if __name__ == '__main__':
    if args.is_train:
        ...
        args.slurm_outfile = "logs/baseline/eeg_freq_baseline_train_1474577.out"
        import re
        import ast
